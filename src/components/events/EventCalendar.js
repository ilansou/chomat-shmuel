...
import { format, isSameDay, isSameMonth, startOfMonth, endOfMonth, eachDayOfInterval, addDays, subDays } from "date-fns";
      ...
      <div className="grid grid-cols-7 gap-1 p-2 bg-gray-100">
        ...
        {days.map((day, idx) => {
          ...
          return (
            <div
              key={idx}
              className={`bg-white p-1 rounded-lg ${isSameMonth(day, selectedDate) ? "" : "opacity-50"} ${
            ...
            >
              <div className="flex justify-between items-center mb-1">
                <span className={`text-lg font-bold ${isSameMonth(day, selectedDate) ? "text-gray-800" : "text-gray-400"}`}>
                  {format(day, "d")}
                </span>
              ...
              <div className="flex-grow overflow-y-auto">
                {getEventsForDay(day).map((eventItem, index) => (
                  <div
                    key={index}
                    className={`text-sm ${categoryColors[eventItem.audienceAge] || 'bg-gray-200'} text-bluck rounded-full px-2 py-0.5 mb-0.5 truncate cursor-pointer hover:opacity-80 transition-all duration-100`}
                    onClick={(e) => {
                      e.stopPropagation();
